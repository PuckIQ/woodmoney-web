      ...
      <div class="container-fluid" role="main">
        <div class="page-header">
            ...
            <div class="row" style="margin-bottom:10px;">
                <div class="col-md-6">
                WOWY: 
                <select name="wowyPlayer" id="wowyPlayer">
                @foreach($teamList as $teammate)
                    <option value="{{$teammate[1]}}">{{$teammate[0]}}</option>
                @endforeach
                </select>
                <input type="hidden" name="defaultPlayerID" id="defaultPlayerID" value="{{$playerID}}">
                <button type="button" class="btn btn-xs btn-success" id="WowySubmit"><strong>View Wowy</strong></button>
                </div>
            </div>

            <?php /*<h4><a href="{{url('wowy')}}/{{$playerID}}">WOWY</a></h4>*/ ?>
            <div class="row">
                <div class="col-md-6">
                Competition : 
              ...
              <div class="row">
                <div class="col-md-12">                  
                  <table class="table table-striped table-bordered table-condensed table-hover" id="puckiq">
                    ...
                    <tbody id="dataTable">
@foreach($teamData as $player)
    @if($player['Conf']=="POTeam" || $player['Conf']=="NPOTeam")
    @else
                     <tr>
                        <td>{{$player['Team']}}</td>
                        <td>{{$player['Comp']}}</td>
